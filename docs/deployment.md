...
0001 through 0020. Migrations 0016–0020 add [compact history and operator-only
...
- `up` builds the collector image, advances the database through all missing
  migrations (0001–0020 on a fresh database), configures runtime role
  passwords, and stages the required collector with Global Top-200 disabled.
  A contract-v1 upgrade uses the bridge collector while migrations 0002–0020
