...
SCHEMA = "step9-live-day-v1"
ADMISSION_STATUS = "pending_final_handoff"
CORE_WINDOWS = 288
WINDOW_MINUTES = 5
SLOT_SECONDS = 60
...
def cmd_start(arguments: argparse.Namespace, db: object | None = None) -> dict:
    ...
    if core_end - core_start != timedelta(hours=24):
        raise Step9Error("bad_interval", "core interval must be exactly 24 hours")
    ...
    header = {
        ...
        "admission_reconciliation": {"status": ADMISSION_STATUS},
...
class Database:
    ...
    def transitions(self, ids: list[int], start: str, end: str) -> list[tuple]:
        ...
        visibility evidence (see ADMISSION_STATUS)."""
        def work(cursor):
...
def build_sample(*, run: dict, index: int, expected_utc: datetime,
    ...
    sample: dict = {
        "schema": SCHEMA, "run_id": run["run_id"], "slot": index,
    ...
    if index % SLOTS_PER_WINDOW == SLOTS_PER_WINDOW - 1:
        ...
        window = {
            ...
            "role": "core",
            "admission_accounting": {"status": ADMISSION_STATUS},
        }
...
def cmd_sample(arguments: argparse.Namespace, hooks=None) -> int:
    ...
    max_slots = hooks.get("max_slots", CORE_SLOTS)
...
def _read_samples(run_dir: Path) -> list[dict]:
    ...
    for index in range(CORE_SLOTS):
        ...
        if payload.get("slot") != index or payload.get("schema") != SCHEMA:
    ...
    extras = sorted(p.name for p in samples_dir.glob("minute-*.json")
                    if p.name > f"minute-{CORE_SLOTS - 1:04d}.json")
    if extras:
        raise Step9Error("sample_overflow", "more than 1440 core samples", gate=True)
    return samples
...
def cmd_finalize(arguments: argparse.Namespace, hooks=None) -> int:
    ...
    try:
        ...
        if len(windows) != CORE_WINDOWS:
            raise Step9Error("window_count",f"expected 288 windows, found {len(windows)}",gate=True)
        ...
        final = {
            "schema": SCHEMA, "run_id": run["run_id"],
...
def _write_manifest(run_dir: Path) -> dict:
    ...
    manifest = {"schema": SCHEMA, "run_id": _load_run(run_dir)["run_id"],
...
def cmd_validate(arguments: argparse.Namespace) -> int:
    ...
    try:
        ...
        windows = [s for s in samples if "window" in s]
        if len(windows) != CORE_WINDOWS:
            raise Step9Error("window_count", "expected exactly 288 core windows",
                             gate=True)
        ...
        if slots != list(range(CORE_SLOTS)):
            raise Step9Error("sample_order", "samples are not exactly 0..1439",
                             gate=True)
        if run.get("admission_reconciliation", {}).get("status") != ADMISSION_STATUS:
            raise Step9Error("admission_frozen",
                             "admission schema must stay pending final handoff")
        ...
        if final.get("admission_reconciliation", {}).get("status") != ADMISSION_STATUS:
            raise Step9Error("admission_frozen",
                             "final.json must not claim complete window accounting")
        reset = final.get("reset", {})
        if reset.get("failure_code") == "reset_handoff_unproven":
...
def cmd_watchdog(arguments: argparse.Namespace, hooks=None) -> int:
    ...
    while iteration < max_iterations:
        ...
        if outcome is not None:
            ...
            result = {"schema": SCHEMA, "run_id": run["run_id"],
