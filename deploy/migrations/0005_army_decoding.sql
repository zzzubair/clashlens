...
ALTER TABLE python_processing_jobs ADD CONSTRAINT python_processing_jobs_input_v2_check CHECK (
    ...
    AND COALESCE(CASE work_type
        ...
        WHEN 'build_army_analytics' THEN (jsonb_typeof(input_json -> 'ranked_day_start') = 'string' AND input_json ->> 'ranked_day_start' ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T05:00:00') OR (jsonb_typeof(input_json -> 'official_season_id') = 'string' AND input_json ->> 'official_season_id' <> '')
